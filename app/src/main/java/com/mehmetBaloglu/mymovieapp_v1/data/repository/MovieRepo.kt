...
class MovieRepo(var apiDao: ApiDao) {
    ...
    suspend fun discoverMovies(
        ...
        voteCount: Int,
        keyword: String?
    ): List<FilmItem> = withContext(Dispatchers.IO) {
    ...
    fun createUserWatchList(): MutableLiveData<List<ForFirebaseResponse>> {
        ...
        db.collection("WatchList").whereEqualTo("email", usersEmailAdress)
    ...
    fun createUserWatchEDList(): MutableLiveData<List<ForFirebaseResponse>> {
        ...
        db.collection("WatchedList").whereEqualTo("email", usersEmailAdress)
            .addSnapshotListener { value, error ->
                if (value != null) {
    ...
    fun deleteFromWatchEDList(context: Context, filmID: String) {
        ...
        db.collection("WatchedList").whereEqualTo("filmID", filmID).get()
                ...
                if (!querySnapshot.isEmpty) {
                    ...
                    db.collection("WatchedList").document(document.id).delete()
                        .addOnSuccessListener {
                            ...
                            Toast.makeText(context, "deleted from WatchedList", Toast.LENGTH_SHORT)
                                .show()
    ...
    fun deleteFromWatchList(context: Context, filmID: String) {
        ...
        db.collection("WatchList").whereEqualTo("filmID", filmID).get()
            .addOnSuccessListener { querySnapshot ->
                if (!querySnapshot.isEmpty) {
                    val document = querySnapshot.documents[0]

                    db.collection("WatchList").document(document.id).delete().addOnSuccessListener {
                        println("Document successfully deleted!")
